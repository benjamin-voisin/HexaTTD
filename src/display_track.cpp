...
void ArcTrack::draw(Layout layout) {
    ...
    if (layout.size.x > 40) {
        ...
        DrawRing(center.to_Vector2(), radius-display_gauge/2-delta, radius+display_gauge/2+delta, angle_min, angle_max, 30, BROWN);
    }
...
void StraighTrack::draw(Layout layout) {
    ...
    if (layout.size.x > 40) {
        ...
        DrawLineEx(src.to_Vector2(), dst.to_Vector2(), display_gauge+2*delta, BROWN);
    }
