...
void DrawTrain::draw() {
    Vector position = _position - (_direction.orthogonal() * _size.x / 2) +
                      (_direction * _size.y / 2);
    DrawRectanglePro({position.x, position.y, _size.x, _size.y}, {0, 0},
                     _direction.angle(), RED);
